...
namespace Project1
{
    public partial class Form1 : Form
    {
        ...
        private void Start_Click(object sender, EventArgs e)
        {
            ...
            l[0].enableButton(level,"Idź do Level 2",flowLayoutPanel1);
        }

        private void Level_Click(object sender, EventArgs e)
        { 
            if(level.Text == "Idź do Level 2")
            {
                if (l[0] is Level1)
                {
                    l1 = (Level1)l[0];
                    ...
                    level.Visible = false;
                    l1.enableButton(level, "Idź do Level 3", flowLayoutPanel1);
                }
            }
            else if(level.Text == "Idź do Level 3")
            {            
                l1.buttonProperties();
                l1.removeButtonKeyboard(flowLayoutPanel1);
                HangMan.resetCounter();
                ...
                l[1].hintLabel(hint);
                level.Visible = false;
                l[1].enableButton(level, "Idź do Level 4", flowLayoutPanel1);

            }
            else if(level.Text == "Idź do Level 4")
            {
                if (l[1] is Level2)
                {
                    l2 = (Level2)l[1];
                    ...
                    level.Visible = false;
                    l2.enableButton(level, "Idź do Level 5", flowLayoutPanel1);
                }
            }
            else if(level.Text == "Idź do Level 5")
            {
                l2.buttonProperties();
                l2.removeButtonKeyboard(flowLayoutPanel1);
                HangMan.resetCounter();
                ...
                l[2].hintLabel(hint);
                level.Visible = false;
                l[2].enableButton(level, "Idź do  Level 6", flowLayoutPanel1);
            }
            else if(level.Text == "Idź do Level 6")
            {
                if(l[2] is Level3)
                {
                    l3 = (Level3)l[2];
                    ...
                    level.Visible = false;
                    l3.enableButton(level, "Koniec gry", flowLayoutPanel1);
                }              
            }
            else
            {
                MessageBox.Show("Twoje punkty: " + HangMan.showScore()+", dobra próba."); 
                Application.Exit();
            }
        }
